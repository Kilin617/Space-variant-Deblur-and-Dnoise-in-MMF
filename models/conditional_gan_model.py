...
class ConditionalGAN(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(
            ...
            not opt.no_dropout, self.gpu_ids, use_parallel, opt.learn_residual, opt.Add_gauss
        )
        if self.isTrain:
        ...
        print('-----------Networks initialized end-----------')

        # write the model to txt file
        expr_dir = os.path.join(opt.checkpoints_dir, opt.name)
        util.mkdirs(expr_dir)
        file_name = os.path.join(expr_dir, 'model.txt')

        with open(file_name, 'wt') as opt_file:
            opt_file.write('------ Networks Saving-------\n')
            opt_file.write('Network Generator:\n %s\n\n\n\n' % (self.netG))
            if self.isTrain:
                opt_file.write('Network Discriminator:\n %s\n' % (self.netD))

            opt_file.write('--------------- End ---------------\n')

    def set_input(self, input):
        AtoB = self.opt.which_direction == 'AtoB' # two image with size(3,256,256) into one image with size(6,256,256)
        # this is for marking the image direction
